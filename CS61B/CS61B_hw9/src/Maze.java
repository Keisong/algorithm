...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    int wallnumbers=horiz*(vert-1)+vert*(horiz-1);
    Walls[] walls=new Walls[wallnumbers];
    int count=0;
    for( i=0;i<horiz;i++){
        for( j=0;j<vert-1;j++){
            walls[count]=new Walls(i,j,true);
            count++;
        }
    }
    for(i=0;i<horiz-1;i++){
        for(j=0;j<vert;j++){
            walls[count]=new Walls(i,j,false);
              count++;           
        }
    }
    for(i=wallnumbers-1;i>0;i--){
        int r=randInt(i);
        Walls template=walls[i];
        walls[i]=walls[r];
        walls[r]=template;
    }
    DisjointSets set=new DisjointSets(this.horiz*this.vert);
    try{
    for(i=0;i<wallnumbers;i++){
        if(walls[i].isHoriz){
            int number1=walls[i].h+walls[i].v*horiz;
            int number2=walls[i].h+(walls[i].v+1)*horiz;
            if(set.find(number1)==set.find(number2)){
                hWalls[walls[i].h][walls[i].v]=true;
            }
            else{
                hWalls[walls[i].h][walls[i].v]=false;
                set.union(set.find(number1),set.find(number2));
            }
        }
        else if(!walls[i].isHoriz){
            int number1=walls[i].h+walls[i].v*horiz;
            int number2=number1+1;
            if(set.find(number1)==set.find(number2))
                vWalls[walls[i].h][walls[i].v]=true;
            else{
                vWalls[walls[i].h][walls[i].v]=false;
                set.union(set.find(number1), set.find(number2));
            }
        }
    }
